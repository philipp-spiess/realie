!function($){
  ...
  var diff_worker = new Worker('/js/diff_worker.js')  // outbound
     ,sync_worker = new Worker('/js/patch_worker.js') // inbound
     ,user_id
     ,predefined_colors = 
     [ '#FFCFEA'
      ,'#E8FF9C'
  ...
  sync_worker.onmessage = function(ev) {
    ...
    if (local_text_change !== '') {
      local_shadow[patching_uuid] = local_shadow_change;
      jQuery(modifying_line).html(local_text_change);
      highlightUserEdit(modifying_line, patch_user_id);
      local_shadow[patching_uuid] = {'content': local_text_change};
    };
  };
  ...
  socket.on('init', function(data) {
    ...
    existingLines.each(function(i) {
      ...
      local_shadow[uuid] = {'content': content};
    });
  ...
  var modifyLine = function(payload){
    ...
    var _local_shadow = local_shadow[uuid]; 
  ...
  var inspectLineChanges = function(i) {
    ...
    var editable_lines = jQuery('#editable_content').children('div').children('p');
    ...
    editable_lines.each(function(i) {
      ...
      if(uuid === undefined || uuid === prev_uuid) {
        jQuery(this).attr('id', 'line' + editable_lines.length);
  ...
  var addUser = function(id){
    var $user = $('<li id="user-' + id + '"><a href="#"></a></li>');
    assigned_colors[id] = predefined_colors.pop();
    $user.find('a')
      .append('<span class="color" style="background-color:' + assigned_colors[id] + '"></span>')
      .append('<span class="name">User-' + id + '</span>');
    $(".users").append($user); 
  };
  ...
  $(function() { 
    ...
    for (var i=0;i<editing_lines.length;i++) {
      console.log(jQuery(editing_lines[i]).attr('data-uuid'));
      ...
      local_shadow[uuid] = { 'content': local_text_value };
    };
